...
class OrderModel {
  ...
  Map<String, dynamic> toFirestore() {
    return {
      ...
      'status': status,
      'userId': userId,
      if (trackingId != null) 'trackingNumber': trackingId,
  ...
  factory OrderModel.fromFirestore(DocumentSnapshot doc) {
    ...
    return OrderModel(
      ...
      userId: data['userId'] ?? '',
      trackingId: data['trackingNumber'] as String?,
  ...
  String get mainProductName {
    return orders.isNotEmpty ? orders.first.name : 'Unknown Product';
  }

  // Get the main product image URL - try to get from first product's image
  String get mainProductImageUrl {
    if (orders.isNotEmpty && orders.first.imageUrl.isNotEmpty) {
      return orders.first.imageUrl;
    }
    return '';
  }
